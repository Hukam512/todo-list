...
═══════════════════════════════════════════════════════════
[MODULE 6: VERIFICATION ENGINE (called by Module 11 Step 2)]
═══════════════════════════════════════════════════════════
BEFORE emitting final MQL4 code, execute this exact block:
<verify_scratchpad>
STRUCTURAL CHECKS:
...
//|  7. No ObjectDelete inside OnCalculate — ever                    |
//|  8. ArraySetAsSeries — always, for every buffer                  |
//|  9. Dynamic Sizing — all pixels derive from Font Size            |
//| 10. Protocol v6.1-ENTERPRISE++ aligned                        |
//+------------------------------------------------------------------+
#property copyright   "Master Architecture Template"
#property link        ""
#property version     "1.10" // Bumped to reflect critical layout fix
#property strict
#property indicator_chart_window
#property indicator_buffers 4
#property indicator_plots   4
...
//--- 2C: Dashboard UI Controls
input ENUM_THEME InpTheme      = THEME_DARK; // Dashboard Theme
input int       InpFontSize    = 10;        // Font Size (MASTER DIMENSION)
// ⚠️ FIX 1 APPLIED: Removed InpRowHeight and InpDashWidth. 
// All layout metrics are now strictly derived from InpFontSize in ResolveTheme()
input int       InpDashX       = 20;        // Dashboard X Position
input int       InpDashY       = 30;        // Dashboard Y Position
input bool      InpShowDash    = true;      // Show Dashboard
...
//--- 5C: Calculated Layout Values
int    g_dashTotalHeight;
int    g_headerHeight;
int    g_dataRowStart;
int    g_rowH;          // ⚠️ FIX 1: Now strictly derived from FontSize
int    g_dashWidth;       // ⚠️ FIX 1: Now strictly derived from FontSize

//--- 5D: Object Name Prefixes
string g_objPrefix = "MA_";
...
void LogChain(string message, ENUM_LOG_LEVEL level = LOG_INFO, string funcName = "")
  {
   ...
   PrintFormat("[%s] [%s] [%s%s] %s",
               timeStr, levelStr, prefix, funcName, message);
  }
   ...
   if(prev_calculated == 0)
     {
      limit = MathMin(rates_total - 1, InpMaxBars);
      LogChain("First run detected. Capping loop to " +
               IntegerToString(limit) + " bars.", LOG_INFO, "CalcLimit");
     }
   else
     {
      if(InpRecalcLastBar)
   ...
   if(ObjectFind(0, fullName) < 0)
     {
      if(!ObjectCreate(0, fullName, OBJ_RECTANGLE_LABEL, 0, 0, 0))
        {
         LogChain("Failed to create rectangle: " + fullName, LOG_ERROR, "DrawRect");
         return;
        }
      ObjectSetInteger(0, fullName, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, fullName, OBJPROP_HIDDEN, true);
   ...
   if(ObjectFind(0, fullName) < 0)
     {
      if(!ObjectCreate(0, fullName, OBJ_LABEL, 0, 0, 0))
        {
         LogChain("Failed to create text: " + fullName, LOG_ERROR, "DrawText");
         return;
        }
      ObjectSetInteger(0, fullName, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, fullName, OBJPROP_HIDDEN, true);
...
//+==================================================================+
//|          SECTION 9: THEME RESOLVER & DYNAMIC SIZING                  |
//+==================================================================+
void ResolveTheme()
  {
   //--- ⚠️ FIX 1: DYNAMIC SIZING (Zero hardcoded pixel values)
   g_rowH         = (int)(InpFontSize * 2.2);
   g_dashWidth    = (int)(InpFontSize * 22.0);
   g_headerHeight = (int)(InpFontSize * 2.8);
   ...
   //--- Final Layout Positions using dynamic variables
   g_dataRowStart = InpDashY + g_headerHeight;
   g_dashTotalHeight = g_headerHeight + (8 * g_rowH);

   LogChain("Theme resolved. Dynamic RowH=" + IntegerToString(g_rowH) + 
            " Width=" + IntegerToString(g_dashWidth), LOG_INFO, "ResolveTheme");
  }
...
//+==================================================================+
//|          PROTOCOL BRIDGE: SECTIONS 14-19                           |
//|                                                                  |
//|  ⚠️ FIX 2 APPLIED: S1-S13 are provided as a starting skeleton.       |
//|  Sections 14-19 are complex and change entirely based on the     |
//|  indicator being built (buffers needed, rows needed, etc.).        |
//|                                                                  |
//|  When the AI executes WORDZIP Protocol Module 11 (Pipeline), it     |
//|  will read the Domain Kernel (Protocol Module 8) and dynamically  |
//|  generate S14-S19 specifically for this indicator type using     |
//|  the Execution Contracts and State Table.                      |
//|                                                                  |
//|  NEVER paste S14-S19 from another file blindly — they must be   |
//|  generated fresh by the AI to match the exact buffers and      |
//|  state variables declared in S1-S13 above.                    |
//|                                                                  |
//|  Reference Protocol Module 8 for exact Section Contracts.         |
//+==================================================================+
...
int OnInit()
  {
   //--- Step 0: CRITICAL - Wipe stale objects first
   CleanupAllObjects();

   LogChain("========== INITIALIZING ==========", LOG_INFO, "OnInit");

   //--- Step 1: Resolve visual theme & dynamic sizes
   ResolveTheme();

   //--- Step 2: Setup indicator buffers
   SetupBuffers();

   //--- Step 3: Set indicator short name
   IndicatorShortName("MasterArch(" +
                      IntegerToString(InpFastPeriod) + "," +
                      IntegerToString(InpSlowPeriod) + "," +
                      IntegerToString(InpSignalPeriod) + ")");
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   if(rates_total != prev_calculated)
     {
      ENUM_SIGNAL newSignal = CalculateSignal(1);
      if(newSignal != SIGNAL_NONE && newSignal != g_prevSignal)
        {
         TriggerAlert(newSignal, 1);
         g_prevSignal = newSignal;
...
//+------------------------------------------------------------------+
//|   MODULE 6: VERIFICATION ENGINE (called by Protocol Module 11 Step 2)|
//+------------------------------------------------------------------+
void RunVerification()
  {
   // Note: This is a placeholder. The actual Module 6 logic runs inside the 
   // AI's <verify_scratchpad> during code generation. 
   // See WORDZIP_PROTOCOL v6.1-ENTERPRISE++ System Prompt for logic.
  }
